...
import { headers, handleUserProfile } from "./services";
import styles from "./styles/allPlayers.module.scss";
import { useRouter } from "next/router";
import moment from "moment";

const AllPlayersComponent = ({ allPlayers }) => {
    ...
    <div className={styles.tableContainer}>
      {allPlayers === undefined ? (
        ...
        <div className="w-full h-2/4 flex items-center justify-center	">
          <p className="text-3xl font-medium text-sky-800">There is no data.</p>
        </div>
      ) : (
        <Table striped bordered hover variant="dark" className={styles.table}>
          ...
          <tbody className={styles.body}>
            {allPlayers?.map((player, index) => (
              <tr
                key={index}
                onClick={() => handleUserProfile(router, player._key)}
                className={styles.tableRow}
              >
                <td className={styles.number}>{index + 1}</td>
                <td>
                  {moment(player.inserted_at).format("D MMM YYYY") || "-"}
                </td>
